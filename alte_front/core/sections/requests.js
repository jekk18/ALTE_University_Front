...
async function getSearchResults(page, filter = {}) {
  try { 
    const response = await axios.get(`${process.env.NEXT_PUBLIC_SERVER_URL}/main-search`, { params: { ...filter, page } });
    return response.data;
  } catch (error) {
    console.error("Error fetching posts:", error);
  }
...
export { getMenus, getPage, getSectionPosts, getComponentPosts, getSchoolProgram, getProgramSchool, getSearchResults }
