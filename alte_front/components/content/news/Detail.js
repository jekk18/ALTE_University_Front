...
const Detail = (props) => {
  ...
  const shared_locale = props?.page?.additional?.shared_locale?.gallery || locale;
  
  let imgUrl = props.page.files?.filter((x) => x.locale === shared_locale && x.type == 'gallery');

  const topicFilter = props?.page?.directories?.filter((x) => x.type_id === 2);

  let st_date = "";
  let [st_day, st_month, st_year] = "";
  let st_monthName = "";

  if (props.page.additional.start_date) {
    [st_day, st_month, st_year] =
      props?.page?.additional?.start_date?.split("/");
    st_date = new Date(st_year, parseInt(st_month) - 1, st_day);
  ...
  if (props.page.additional.end_date) { 
    [end_day, end_month, end_year] = props.page.additional.end_date.split("/");
    end_date = new Date(end_year, parseInt(end_month) - 1, end_day);
